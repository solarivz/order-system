...
def add_order():
    # Добавляем проверку на пустые поля ввода
    if not customer_name_entry.get() or not order_details_entry.get():
...
def complete_order():
    ...
    if selected_item:
        order_id = tree.item(selected_item[0], 'values')[0]  # Исправлено извлечение значения 'id'
        conn = sqlite3.connect('business_orders.db')
        cur = conn.cursor()
